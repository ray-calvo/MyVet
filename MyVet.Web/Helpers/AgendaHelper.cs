...
namespace MyVet.Web.Helpers
{
    public class AgendaHelper : IAgendaHelper
    {
        ...
        public async Task AddDaysAsync(int days)
        {
            ...
            {
                initialDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 9, 0, 0);
            }
            else
            {
                var agenda = _dataContext.Agendas.LastOrDefault();
                initialDate = new DateTime(agenda.Date.Year, agenda.Date.Month, agenda.Date.AddDays(1).Day, 9, 0, 0);
            }
